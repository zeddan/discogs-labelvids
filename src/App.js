...
class App extends Component {
  ...
  playPrevious = () => {
    ...
    if (prevIdx < 0) prevIdx = this.state.videos.length - 1;
  ...
  scrollToPlaying = () => {
    setTimeout(() => {
      let pos =
        document.getElementsByClassName("now-playing")[0].parentElement
          .offsetTop;
  ...
  search = async (labelId) => {
    ...
    const res = await axios
      .get(`https://api.discogs.com/labels/${labelId}/releases?${token}`)
      .catch(this.logError);
  ...
  logError = (error) => {
    if (error.response) {
      ...
      console.error(
        "discogs_labelvids: Request was made but no response was received"
      );
      console.error(error.request);
      console.error(error.message);
      alert(
        `${error.message}: Probably too many requests was sent to Discogs too fast`
      );
    } else {
      console.error(
        "discogs_labelvids: Something happened in setting up the request that triggered an Error"
      );
    ...
    return (
      <div className="App">
        <div id="search-container">
          <input
            id="search-input"
            placeholder="Enter label's discogs-id"
          ></input>
          <div id="search-button" onClick={this.onSearchButtonClicked}>
            Search
          </div>
        </div>
        <div id="player"></div>
        {this.state.videos.length > 0 ? (
          <div id="player-control">
            ...
            <a
              href={this.state.currentUrl}
              target="_blank"
              rel="noopener noreferrer"
            >
              <img
                id="discogs-icon"
                title={this.state.currentUrl}
                alt={this.state.currentUrl}
              ...
              return (
                <div className="list-item" key={i}>
                  <div
                    ...
                    style={this.setThumbnailBg(video.uri)}
                  ></div>
                  <div
